...
class VendorDeliveryAddressController extends Controller
{
  public function index(Request $request)
    {
        $vendor   = $request->user();
        $addresses = $vendor->deliveryAddresses()->orderByDesc('is_default')->get();
    ...
    public function store(Request $request)
    {
        ...
        $data['is_default'] = ! $vendor->deliveryAddresses()->exists();
...
{
    abort_unless($address->vendor_id === $request->user()->id, 403);
